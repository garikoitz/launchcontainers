...
def link(
    ...
    force: bool = typer.Option(
        False, "--force", "-f", help="Overwrite existing symlinks"
    ),
...
def check(
    ...
    for item in symlink_map:
        console.print(
            f"  {item['original'].name} - {item['datetime'].strftime('%H:%M:%S')} - {item['task']}"
        )
...
def rename(
    ...
    for m in matches:
        ...
        console.print("[yellow]Renaming:[/yellow]")
        console.print(f"  {old_nii.name} → {new_nii.name}")
        console.print(f"  {old_json.name} → {new_json.name}")

        pairs = [(old_nii, new_nii)]
        if old_json.exists():
            pairs.append((old_json, new_json))

        try:
            atomic_rename_pairs(pairs, dry_run=dry_run)
            if not dry_run:
