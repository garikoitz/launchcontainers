...
def match_bids_files(mat_map, bids_dir: Path, sub: str, ses: str, max_gap: int = 200):
    ...
    # Debug the sbref
    console.print(f"[blue]SBRef files:[/blue]")
    ...
    def match_file_to_mat(nii_file, file_type): 
        ...
        if not file_dt: 
            console.print(f" [{file_type}] {nii_file.name}: NO ACQUISITION TIME in JSON") 
            return None 
        ...
        if best_match:
            console.print(f"  [{file_type}] {nii_file.name}: {file_task}-{file_run} ({file_dt.strftime('%H:%M:%S')}) → {best_match['task']}-{best_match['run']} (diff: {int(min_diff.total_seconds())}s)")
            return {
                'nii_file': nii_file,
                'json_file': json_file,
                'file_task': file_task,
                ...
                'file_type': file_type,
                'needs_rename': not (file_task == best_match['task'] and file_run == best_match['run'])
            }
...
def check(
    ...
    for m in matches:
        # Bold status
        if m['bold']:
            bold_str = f"[green]✓[/green] {m['bold']['file_task']}-{m['bold']['file_run']:02d}" if not m['bold']['needs_rename'] else f"[red]✗[/red] {m['bold']['file_task']}-{m['bold']['file_run']:02d}"
        else:
            bold_str = "[red]Missing[/red]"
        
        # SBRef status
        if m['sbref']:
            sbref_str = f"[green]✓[/green] {m['sbref']['file_task']}-{m['sbref']['file_run']:02d}" if not m['sbref']['needs_rename'] else f"[red]✗[/red] {m['sbref']['file_task']}-{m['sbref']['file_run']:02d}"
        else:
...
def rename(
    ...
    for m in matches:
        console.print(f"[cyan]Processing MAT: {m['mat_task']} run-{m['mat_run']:02d}[/cyan]")
