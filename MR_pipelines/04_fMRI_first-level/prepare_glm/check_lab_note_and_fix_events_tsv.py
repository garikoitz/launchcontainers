...
if __name__ == "__main__":
    ...
    xlsx_file = "/bcbl/home/public/Gari/VOTCLOC/main_exp/VOTCLOC_subses_list.xlsx"
    # 🔹 Load the Excel file
    ...
    for sheet_name in xls.sheet_names:
        if sheet_name.startswith("sub-"):  # Process sheets named sub-xx
            ...
            df["sub"] = (
                df["sub"].astype(int).astype(str).str.zfill(2)
            ...
            if all(
                col in df.columns
                for col in ["sub", "ses", "date", "protocol_name", "quality_mark"]
            ):
                # 🔹 Filter rows where "protocol_name" contains "rerun" or "skip"
                filtered_rows = df[
                    df["protocol_name"].str.contains("rerun", case=False, na=False)
                    & ~df["protocol_name"].str.contains(
                        r"qmri|T1", case=False, na=False
                    )
                ]  # | df['quality_mark'].str.contains('failed', case=False, na=False)]
    ...
    for _, row in df_filtered.iterrows():
        ...
        if run_part and rerun_part and task_part:
            ...
            try:
                ...
                target_event = source_event.replace(
                    f"run-{rerun_number}", f"run-{run_number}"
                )
                # Ensure source file exists before linking
                if os.path.exists(source_event):
                    # Avoid overwriting existing files
                    if not (
                        os.path.exists(target_event) or os.path.islink(target_event)
                    ):
