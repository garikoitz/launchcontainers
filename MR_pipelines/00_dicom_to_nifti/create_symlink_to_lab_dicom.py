...
@app.command()
def dcm_dir_summary(lab_project_dir: Path, 
            ...
            for dirpath, subdir_names, file_names in os.walk(ses_dir_path):
                ...
                if not subdir_names and file_names:
                    ...
                    if len(file_names) > 209:
                        print(f"WARNING !!! the number of file_names of this ses is not correct {ses_dir_name}")
                        func_correct = 0
                    # get the acq date   
                    if not "Phoenix" in dirpath:
    ...
    dcm_sum_df = pd.DataFrame(rows, columns=[
                                        "lab_project_dir",
                                        "dir_name",  
                                        "levels_from_top", 
                                        "number_of_protocal", 
...
def main():
    ...
    dcm_merged['_date_list'] = dcm_merged['acq_date'].apply(lambda x: x if isinstance(x, set) else set([x]))
    # this will not work because the reading of csv will not perceve the class set
    dcm_mergedx = dcm_merged.explode("_date_list", ignore_index = True).rename(columns = {"_date_list": "date"})
    ...
    dcm_mergedx[["dcm_sub", "dcm_ses"]] = dcm_mergedx["sub_ses"].str.extract(r"sub-(\d{2})_ses-(\d{2})")
    dcm_mergedx[['ses_suffix','ses_label']] = dcm_mergedx['sub_ses'].apply(lambda x: pd.Series(get_ses_suffix(x)))

    merged = dcm_mergedx.merge(lab_note_df, 
            ...
            try:
                ...
                if len(info) >1:
                    ...
                    if len(info["lab_project_dir"].unique())>1:
                        idx_to_drop = info.index[~ info['lab_project_dir'].apply(lambda p : str(p).endswith("images"))]
                        info = info.drop(idx_to_drop)
                    # we need to compare the info
                    if len(info) > 1:
                        print(f'$$need manual correction sub={sub},ses={ses}')
                        print(info[['sub','ses','date','ses_label','ses_suffix','session_correct']])
                    else:
                ...
                # try to find the sub and ses in the df merged
                info_dcm=dcm_mergedx.groupby(['dcm_sub','dcm_ses']).get_group((sub,ses))
                if info_dcm[info_dcm['ses_label']=='normal']['dcm_sub']=='02':
                    print(f"#sub-{sub}_ses-{ses} didn't get it")
                else:
                    if info_dcm['session_correct'].item()==0:
                        print(f'*session have mismatch functional run sub={sub},ses={ses}')
