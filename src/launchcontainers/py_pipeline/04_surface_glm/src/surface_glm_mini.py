...
def glm_l1(subject, session, use_smoothed=False, sm=None):
    ...
    outdir = op.join(bids, "derivatives", f"l1_surface_{sm}fwhm", f'{subject}')
    ...
    for hemi in hemis:
        ...
        for run_num in run_nums:
            ...
            for index, (contrast_id, contrast_val) in enumerate(contrasts.items()):
                ...
                outname_base_run = f"sub-{subject}_ses-{session}_task-{task}_{run}_hemi-{hemi}_space-{space}_contrast-{contrast_id}_stat-X_statmap.func.gii"
                if use_smoothed:
...
def main():
    ...
    sm=parser_dict['sm']
    glm_l1(subject, session, use_smoothed, sm)
    return
