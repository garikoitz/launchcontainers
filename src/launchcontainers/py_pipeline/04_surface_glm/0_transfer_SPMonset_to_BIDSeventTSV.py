...
def save_onset_to_bids_event(layout,sub, ses, task, run_num, df):
    fname_event=layout.get(subject=sub, session=ses, task=task, run="{:02d}".format(run_num),  suffix='events', extension='.tsv')[0].filename
    storepath_event=layout.get(subject=sub, session=ses, task=task, run="{:02d}".format(run_num),  suffix='events', extension='.tsv')[0].path
